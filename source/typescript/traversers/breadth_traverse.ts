...
class breadthTraverser<T, K extends keyof T, B> extends Traverser<T, K, B & { traverse_state: TraverseState; }> {
 ...
 */
export function breadthTraverse<T, K extends keyof T>(node: T, key: K, yield_on_exit_only: boolean = false, max_depth: number = Infinity) {

    max_depth = Math.max(0, Math.min(100000, max_depth - 1));

    return new breadthTraverser<T, K, MetaRoot<T, K>>(node, key, { depth: 0, key, index: 0, parent: null, next: null, prev: null, traverse_state: 0 }, yield_on_exit_only, max_depth);
}
