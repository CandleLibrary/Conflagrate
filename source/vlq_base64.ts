...
export function encodeVLQBase64(number) {
    ...
    const m_s = segments
        .reduce((r, s, i) => (s & 0x3F) ? i : r, 0), base64 = segments
...
export function decodeVLQBase64Array(string): any {
    const array = Array.from(string).map(e => base64LU.get(<string>e)), out_array = [];
    let start = 0;
    for (let i = 0; i < array.length; i++) {
        if (!(array[i] & 0x20)) {
            const VLQ = array.slice(start, i + 1), ls = VLQ[0], sign = 1 - ((ls & 1) << 1), val = VLQ
                .slice(1)
